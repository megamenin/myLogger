...
class AnohovLogger extends AbstractLogger implements LoggerInterface
{
    ...
    private function contextToString(array $context = array())
    {
        ...
        foreach ($context as $key => $val) {
            $str .= ' ['.$key.']: '.$this->messageToString($val).';';
        }
    ...
    public function log($level, $message, array $context = array())
    {
        switch ($level) {
            ...
            case LogLevel::DEBUG:
                ...
                $template = '['.strtoupper($level).' MESSAGE]: "'.$this->messageToString($message)
                    .'", [CONTEXT]: '.$contextToStr.'';

                switch (self::$LOG_TARGET) {
                case AnohovLoggerTarget::DATABASE:
                    $query = 'INSERT INTO `'.self::$LOG_TABLENAME.
                        '` VALUES (NULL, "'.addslashes($template).'", "'.$this->getDate().'");';
                    if (!$this->connect->query($query)) {
                        throw new \Exception('invalid query');
                    }
                    break;
                case AnohovLoggerTarget::FILE:
                    $template = '['.$this->getDate().']: '.$template."\n\n";
                    fwrite($this->fp, $template);
                    break;
                case AnohovLoggerTarget::STDOUT:
                    $template = '['.$this->getDate().']: '.$template."\n\n";
                    fputs($this->fp, $template);
                    break;
                }
                break;
            default:
    ...
    public static function setLogSetting(array $setting)
    {
        if (self::$logger == null) {
            self::$logger = new self($setting);
        } else {
            switch (self::$LOG_TARGET) {
            case AnohovLoggerTarget::DATABASE:
